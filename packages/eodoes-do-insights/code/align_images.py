...
if __name__ == "__main__":
    ...
    parser.add_argument('--marks_path', default='shape_predictor_68_face_landmarks.dat.bz2', help='dlib landmarks http://dlib.net/files/')
    parser.add_argument('--marks_url', default='http://dlib.net/files/shape_predictor_68_face_landmarks.dat.bz2', help='dlib landmarks http://dlib.net/files/')
    ...
    def open_file_or_url(file_or_url):
        if dnnlib.util.is_url(file_or_url):
            return unpack_bz2(get_file(file_or_url, LANDMARKS_MODEL_URL, cache_subdir='temp'))
        return unpack_bz2(file_or_url)

    def load_marks(file_or_url):
        return open_file_or_url(file_or_url)

    marks_uri=args.marks_url
    if args.marks_path:
        marks_uri=args.marks_path

    landmarks_model_path = load_marks(marks_uri)
    # _e_
